...
class BurstIqController extends Controller
{
    ...
    function find(Request $request)
    {
        ...
        if (($I != 'search-input') and ($I != 'provider-search-input'))
            ...
            $where = $where . ' AND asset.customer_product_id='.env('CUSTOMER_PRODUCT_ID'); // .session('provider');
    ...
    public function patientConsented (Request $request) {

        $Q = isset($request->id)? $request->id: Auth::user()->id;
        $P = new PatientProfile();
        
        $where = "WHERE asset.id=$Q";
        //         if (!$P->setPatientConsented(1)) {
        if (!$P->find($where)) {
        ...
        $insurances =  $P->getInsurances();
        array_walk_recursive($insurances, function(&$item){
            if(is_object($item)) $item = (array)$item;
        });
            
            $Race = $P->getRace();
            $BirthSex = $P->getBirthSex();
            $P->setPatientConsented(true);
            $P->asset = $P;
            
            $post = $P->make($P);
            $post['insurance'] = $insurances;
            $post['race'] = $Race;
            $post['birth_sex'] = $BirthSex;
            unset($post['insurances']);
            $post['customer_product_id'] = [env('CUSTOMER_PRODUCT_ID')];
            $burstIq = new BurstIq;
            $burstIq->setData($post);
            $burstIq->chain = "patient_profile";
            $burstIq->view  = "biq/patient_profile";
            $response = $burstIq->savePatientConsented($post);
            if ($response->status == 200) {
    
                if(Auth::user()->checkRole('provider')) {
                    return redirect("/barcode-image/$Q");
